  ...
  const handleFirmSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Firm Exit 🥗. Only one firm can be added")
      }
      
      
      else {
        console.error('Backend Error:', data.message || 'Failed to add firm');
        alert(data.message || 'Failed to add firm');
      }
      console.log("this is firmId",data.firmId);
      const firmIdvar=data.firmId;
      localStorage.setItem('firmId',firmIdvar)
    } catch (error) {
